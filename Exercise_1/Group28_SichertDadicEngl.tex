...
All the other values are numerical, therefore we will try different scaling strategies.
 ...
 namely the MinMaxScaler, the StandardScaler, the RobustScaler, the power


 \textbf{Census Income:} This dataset hat missing values, but only for categorical values.
 Those values are labeled with '?'. We will try the same two strategies as for the \textit{Congressional Voting} dataset.
