...
\textbf{Linear Support Vector Classifier:}
...
Later we will see that small changes in \textsf{C} do not vary result too much so we choose \textsf{C}=0.00946 for Amazon and \textsf{C} =1.2 for congressional voting. Taking the other values from table \ref{table:tu_hyp_svc_ac} for congressional voting we get an F1 score of 0.951 which is pretty good and we will use these values.
