...
\section{Ridge Classifier}

\begin{table}[h!]
    \centering
    \begin{tabular}{|c|c|c|c|c|}
    \hline
    Dataset & \textbf{alpha} & \textbf{fit\_intercept} &  \textbf{class\_weight} &  accuracy \\
    \hline
...
\begin{table}[h!]
    ...
    \begin{tabular}{|c|c|c|c|c|}
    \hline
    Dataset & \textbf{alpha} & \textbf{fit\_intercept} &  \textbf{class\_weight} & F1/Recall \\
    \hline
    \multicolumn{5}{c}{\vspace{-0.4cm}} \\ % Adds vertical space
    \hline
    congressional voting & 0 & True & None & 0.963 \\%20241111_122418
...
For congressional voting and amazon the values in table \ref{table:tu_hyp_ridge_ac} and table \ref{table:tu_hyp_svc_f1} coincide so those are the best settings. 
...
Linear SVC sensitivity. In figure \ref{fig:sensitivity svc diabetes} we can see, that for very small values of C the two scores diverge heavily. To get the best Recall with still decent accuracy we chose classweight balanced fit\_intercept True and C=0.0452. This gives us an accuracy of 0.747 and a Recall of 0.734.%20241113_110548 sensitivity analysis
...
By looking at figure \ref{fig:sensitivity ridge diabetes} we can see that we have to set class\_weight to 'balanced' and fit\_intercept to False when favoring Recall over accuracy. In that case the best value for alpha is 0. In that case we get an accuracy of 0.737 and a Recall of 0.797.%20241113_115258 sensitivity analysis
